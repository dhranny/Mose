 ...
 * transactions.</p>
 */
public class RiskAnalyzeer implements RiskAnalysis {
    private static final Logger log = LoggerFactory.getLogger(RiskAnalyzeer.class);
    ...
    private RiskAnalyzeer(Wallet wallet, Transaction tx, List<Transaction> dependencies) {
        this.tx = tx;
    ...
    public static class Analyzer implements RiskAnalysis.Analyzer {
        @Override
        public RiskAnalyzeer create(Wallet wallet, Transaction tx, List<Transaction> dependencies) {
            return new RiskAnalyzeer(wallet, tx, dependencies);
        }
    }
